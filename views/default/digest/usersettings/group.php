	...
	if($user = get_loggedin_user()){
		...
		if(trigger_plugin_hook("digest", "group", array("group" => $group), true)){
			if($group->isMember($user->getGUID())){
				...
				$form_body .= ">" . sprintf(elgg_echo("digest:interval:default"), elgg_echo("digest:interval:" . $group_interval)) . "</option>\n";
...
<div id="owner_block_digest_group" title="<?php echo elgg_echo("digest:usersettings:group:setting"); ?>">
	<?php 
	 	echo elgg_view("output/url", array("href" => $vars["url"] . "pg/digest/user/" . $user->username, "text" => elgg_echo("digest:usersettings:group:more")));
		echo "<br />";
